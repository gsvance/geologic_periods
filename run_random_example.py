...
MIN_N_STUDENTS, MAX_N_STUDENTS = (1, 120)
MIN_N_TOPICS, MAX_N_TOPICS = (1, 30)
MIN_N_CHOICES, MAX_N_CHOICES = (1, 6)
MIN_N_HIGH_PRIORITY, MAX_N_HIGH_PRIORITY = (0, 12)
PROB_NONE_CHOICE = 1.0 / 150
...
if n_high_priority == 0 and bool(random.randrange(2)):
    report = solve_geologic_periods_problem(students, topics, choices,
                                            solver_output=True)
else:
    report = solve_geologic_periods_problem(students, topics, choices,
                                            high_priority, solver_output=True)

print(report)
print(f'RANDOMIZATION: {n_students} students ({n_high_priority}'
